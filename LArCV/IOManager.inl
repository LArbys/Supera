...
namespace larcv {
  ...
  size_t IOManager<T>::register_producer(const std::string& name)
  {
    ...
    _product_v.push_back(data_ptr);
    
    if(_io_mode != kWRITE) {
      LARCV_INFO() << "kREAD/kBOTH mode: creating an input TChain" << std::endl;
      LARCV_DEBUG() << "Branch name: " << br_name << " data pointer: " << data_ptr << std::endl;
    ...
    if(_io_mode != kREAD) {
      ...
      out_tree_ptr->Branch(br_name.c_str(), "std::vector<larcv::Image2D>", &(_product_v.back()));
  ...
  void IOManager<T>::prepare_input()
  {
    ...
    for(auto const& t : _in_tree_v) { _tree_entries = (_tree_entries < (size_t)(t->GetEntries()) ? _tree_entries : (size_t)(t->GetEntries())); }
  ...
  bool IOManager<T>::save_entry()
  {
    ...
    LARCV_INFO() << "Saving new entry: " << _tree_entries+1 << std::endl;
  ...
  void IOManager<T>::finalize()
  {
    ...
    if(_io_mode != kWRITE) {
      ...
      for(auto& t : _in_tree_v) {delete t;};
    }

...
class YoloWire : public art::EDAnalyzer {
public:
  explicit YoloWire(fhicl::ParameterSet const & p);
  // The destructor generated by the compiler is fine for classes
  // without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  YoloWire(YoloWire const &) = delete;
  YoloWire(YoloWire &&) = delete;
  YoloWire & operator = (YoloWire const &) = delete;
  YoloWire & operator = (YoloWire &&) = delete;
...
YoloWire::YoloWire(fhicl::ParameterSet const & p)
  : EDAnalyzer(p)
...
void YoloWire::beginJob() {
}

std::vector<larcaffe::RangeArray_t> YoloWire::setEventCrop(const art::Event& e) 
{
  art::ServiceHandle<geo::Geometry> geom;
...
std::vector<larcaffe::RangeArray_t> YoloWire::findBoundingBoxes(const art::Event& e) 
{
...
void YoloWire::getMCTruth( art::Event const & e ) {
...
void YoloWire::analyze(art::Event const & e)
{
  ...
  if(!_producer_v[0].empty()) {
	...
	<< "Missing Wires by " << _producer_v[0] << " Skipping." << std::endl;
	...
	<< "Extracting " << region_v.size() << " images for Wire " << std::endl;
      ...
      // loop through cropping regions (should only be one for YoloWire Module)
	...
	m_nfilledboxes++;
      ...
      m_ImageTree->Fill();
    }// end of if Wires Availale    
  }// if Wires producer has been specified
  _time_prof_v[kANALYZE_TOTAL] += pWatchAnalyze.RealTime();
  _event_counter += 1;
}

void YoloWire::endJob() {
...
void YoloWire::setupTrees(int nplanes) {
  // ImageTree
  // ---------
...
void YoloWire::clearBoundingBoxes() {

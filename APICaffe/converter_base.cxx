#ifndef __APICAFFE_CONVERTER_BASE_CXX__
#define __APICAFFE_CONVERTER_BASE_CXX__
...
namespace larcv {
  ...
  namespace larcaffe {
    ...
    converter_base::converter_base(const std::string name) : larcv_base(name)
    {}
	...
	throw larbys();
      ...
      if( _data.width() <= (long long)wire ) {
	LARCV_CRITICAL()
	  << "Wire " << wire << " cannot fit in defined data size (width=" << _data.width() << ")" << std::endl;
	throw larbys();
      }
      
      if( (_data.height() * _data.width()) < (_data.height() * (long long)wire + index_offset + num_samples) ) {
	LARCV_CRITICAL()
    ...
    void converter_base::copy_data(const Image2D& img) {
